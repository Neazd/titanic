...
if os.path.exists(filename):
    try:
        with open(filename, 'rb') as file:
            ...
            st.success("Model załadowany za pomocą pickle.")
    except Exception as e:
        st.error(f"Nie udało się załadować modelu za pomocą pickle: {e}")
else:
    st.error(f"Plik {filename} nie został znaleziony. Upewnij się, że plik istnieje w ścieżce {filename}.")
...
def main():
    ...
    if model:
        ...
        with prediction:
            st.error("Model nie został załadowany. Sprawdź plik modelu.")
